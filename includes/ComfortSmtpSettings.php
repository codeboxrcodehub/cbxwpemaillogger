...
class ComfortSmtpSettings
{
    ...
    function sanitize_options($options)
    {
        ...
        foreach ($options as $option_slug => $option_value) {
            ...
            switch ($field_type) {
                ...
                case 'repeat':
                    if (is_array($option_value)) {
                        $options[$option_slug] = $this->sanitize_repeat($option_value);
                    } else {
                        $options[$option_slug] = [];
                    }
                    break;
    ...
    public function get_field_description($args, $element_class = '')
    {
        if ( ! empty($args['desc'])) {
            $field_id         = $args['id'];
            $desc_extra_class = ($element_class != '') ? ' description_'.esc_attr($element_class) : '';
    ...
    /**
     * Displays a callback_radio a settings field
     *
     * @param  array  $args
     * @param $value
     *
    ...
    function callback_multicheck($args, $value = null)
    {
        ...
        $html = '<p class="grouped gapless grouped_buttons checkbox_fields_check_actions"><a href="#" class="button primary checkbox_fields_check_action_call">'.esc_html__('Check All', 'cbxwpemaillogger').'</a><a href="#" class="button outline checkbox_fields_check_action_ucall">'.esc_html__('Uncheck All', 'cbxwpemaillogger').'</a></p>';
    ...
    function callback_page($args)
    {
        ...
        $value         = $this->get_field($args['id'], $args['section'], absint($args['default']));
        $size          = isset($args['size']) && ! is_null($args['size']) ? $args['size'] : 'regular selecttwo-select';
        $check_content = isset($args['check_content']) && ! is_null($args['check_content']) ? $args['check_content'] : '';

        $allow_clear = isset($args['allow_clear']) ? absint($args['allow_clear']) : 0;
        ...
        if ($value > 0) {
            if ($check_content !== '') {
                ...
                if ($page !== null && has_shortcode($page->post_content, $check_content)) {
        ...
        if ($page_shortcode_note !== '') {
            $html .= '<p class="description_note '.esc_attr($shortcode_found_class).'">'.wp_kses_post($page_shortcode_note).'</p>';
        }
    ...
    function callback_repeat($args){
        ...
        $allow_new = isset($args['allow_new']) ? absint($args['allow_new']) : 0;
        $value     = $this->get_field($args['id'], $args['section'], $args['default']);
        ...
        if (is_array($fields) & sizeof($fields) > 0) {
            foreach ($value as $val) {
                ...
                foreach ($fields as $field) {
                    ...
                    $args_t['optgroup']          = isset($field['optgroup']) ? absint($field['optgroup']) : 0;
                    $args_t['sortable']          = isset($field['sortable']) ? absint($field['sortable']) : 0;
    ...
    /**
     * Recursively sanitize values inside repeatable fields.
     *
     * @param mixed $value
     *
     * @return mixed
     */
    public function sanitize_repeat($value)
    {
        if (is_array($value)) {
            foreach ($value as $key => $item) {
                $value[$key] = $this->sanitize_repeat($item);
            }
    ...
    function show_forms()
    {
        ...
        <div id="setting-tabs-contents">
            <div id="global_setting_group_actions" class="mb-0">
                <?php do_action('comfortsmtp_setting_group_actions_start'); ?>
                ...
                <?php do_action('comfortsmtp_setting_group_actions_end'); ?>
