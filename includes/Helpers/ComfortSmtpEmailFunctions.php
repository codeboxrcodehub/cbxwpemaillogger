...
if ( ! function_exists( 'cbxwpemaillogger_rgb_from_hex' ) ) {
	 ...
	 * @return array
	 */
	function cbxwpemaillogger_rgb_from_hex( $color ) {
...
if ( ! function_exists( 'cbxwpemaillogger_hex_darker' ) ) {
	 ...
	 * @return string
	 */
	function cbxwpemaillogger_hex_darker( $color, $factor = 30 ) {
		$base  = cbxwpemaillogger_rgb_from_hex( $color );
		$color = '#';

		foreach ( $base as $k => $v ) {
			$amount      = $v / 100;
			$amount      = cbxwpemaillogger_round( $amount * $factor );
...
if ( ! function_exists( 'cbxwpemaillogger_hex_lighter' ) ) {
	 ...
	 * @return string
	 */
	function cbxwpemaillogger_hex_lighter( $color, $factor = 30 ) {
		$base  = cbxwpemaillogger_rgb_from_hex( $color );
		$color = '#';
...
if ( ! function_exists( 'cbxwpemaillogger_hex_is_light' ) ) {
	 ...
	 */
	function cbxwpemaillogger_hex_is_light( $color ) {
...
if ( ! function_exists( 'cbxwpemaillogger_light_or_dark' ) ) {
	 ...
	 * @return string
	 */
	function cbxwpemaillogger_light_or_dark( $color, $dark = '#000000', $light = '#FFFFFF' ) {
		return cbxwpemaillogger_hex_is_light( $color ) ? $dark : $light;
	}
}

if ( ! function_exists( 'cbxwpemaillogger_format_hex' ) ) {
	 ...
	 * @return string|null
	 */
	function cbxwpemaillogger_format_hex( $hex ) {
 ...
 */
function cbxwpemaillogger_round( $val, int $precision = 0, int $mode = PHP_ROUND_HALF_UP ): float {
	if ( ! is_numeric( $val ) ) {
		$val = floatval( $val );
	}
 ...
 * @return float
 */
function cbxwpemaillogger_array_sum( array $arr ): float {
