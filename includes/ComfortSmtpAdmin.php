...
class ComfortSmtpAdmin {
	...
	public function phpmailer_init_extend( $phpmailer ) {
		...
		$table_comfortsmtp = $wpdb->prefix . 'cbxwpemaillogger_log';
	...
	public function pro_addon_compatibility_campaign() {
		...
		if ( in_array( 'cbxwpemailloggerpro/cbxwpemailloggerpro.php', apply_filters( 'active_plugins',
				get_option( 'active_plugins' ) ) ) || defined( 'COMFORTSMTPPROADDON_PLUGIN_NAME' ) ) {
			//plugin is activated

			$plugin_version = COMFORTSMTPPRO_PLUGIN_VERSION;
	...
	public function plugin_row_meta( $links_array, $plugin_file_name, $plugin_data, $status ) {
		if ( strpos( $plugin_file_name, COMFORTSMTP_BASE_NAME ) !== false ) {
			...
			if ( in_array( 'cbxwpemailloggerpro/cbxwpemailloggerpro.php', apply_filters( 'active_plugins',
					get_option( 'active_plugins' ) ) ) || defined( 'CBXWPEMAILLOGGERPROADDON_PLUGIN_NAME' ) ) {
